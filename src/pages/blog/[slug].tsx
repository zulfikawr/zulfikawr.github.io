    ...
    <div className="space-y-4 px-4">
      ...
      <Transition>
        <Head>
          ...
          <meta
            name="theme-color"
            content={post.hidden ? '#ebb305' : '#171717'}
          />
        </Head>
        ...
        {post.hidden && (
          <div className="bg-yellow-500 text-yellow-900 rounded-md py-2 px-4">
            <p>
              Hey! This post is hidden! Please don't share the link for now...
            </p>
          </div>
        )}
        ...
        <div
          className={`flex ${tocVisible ? 'justify-between' : 'justify-center'} px-2 md:px-4`}
        >
          <div
            className={`md:flex ${tocVisible ? 'justify-start' : 'justify-center'} w-full md:custom-card rounded-xl`}
          >
            <main
              className={`prose prose-hr:border-neutral-200 dark:prose-hr:border-neutral-800 prose-blue prose-img:rounded-md prose-img:w-full dark:prose-invert ${tocVisible ? 'md:max-w-4xl' : 'md:w-full'}`}
            >
              {post.render()}
            </main>
          </div>
